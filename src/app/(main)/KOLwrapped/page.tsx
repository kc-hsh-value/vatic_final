...
//                 value={xUsername}
//                 onChange={(e) => {
//                   // allow users to paste "@elonmusk" or "elonmusk"
//                   const v = e.target.value.trim();
//                   setXUsername(v.startsWith("@") ? v.slice(1) : v);
//                 }}
...
//                 spellCheck={false}
//                 inputMode="text"
//                 className="w-full pl-8 pr-4 py-3 rounded-xl bg-zinc-900 border border-zinc-800 text-sm
...
//             Use the X handle (not displayed username). Example: <span className="font-mono text-zinc-300">@elonmusk</span>
//           </p>
//         </form>
...
function WrappedView({
  ...
  const xEnabled = showX;
  const pmEnabled = showPolymarket && !!pm?.linked;
  const bothEnabled = xEnabled && pmEnabled;
  const noneEnabled = !xEnabled && !pmEnabled;
  ...
  return (
    <div className={containerClass}>
      ...
      >
        {noneEnabled ? (
          <div className={cn(cardBaseClass, "p-6 text-center text-zinc-400")}>
            Enable X and/or Polymarket to render an export card.
          </div>
        ) : null}
...
export default function KolWrappedPage() {
  ...
  async function downloadPng(type: "desktop" | "mobile") {
    try {
      ...
      const parts = [showX ? "x" : null, showPolymarket ? "pm" : null].filter(Boolean).join("-") || "none";

      const a = document.createElement("a");
      a.href = dataUrl;
      a.download = `vatic-kol-wrapped-${submitted ?? "user"}-${type}-${parts}.png`;
      a.click();
    } catch (e: any) {
      setStage("");
      setError(e?.message ? String(e.message) : "PNG export failed.");
  ...
  // Optional nicety: if PM isn't linked, default Polymarket toggle off (but don't force it)
  useEffect(() => {
    if (data && pm?.linked === false) {
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col items-center py-12 px-4 font-sans selection:bg-indigo-500/30">
      ...
      <div className="w-full max-w-2xl mb-12">
        ...
        {/* NEW: toggles (only show once we have data) */}
        {data && (
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            >
              {showPolymarket ? "✓ " : ""}Include Polymarket
            </button>

            {pm?.linked === false && showPolymarket && (
              <span className="px-3 py-2 rounded-xl border border-yellow-500/20 bg-yellow-500/10 text-yellow-200 text-xs">
                Not linked to Polymarket.
              </span>
            )}
          </div>
        )}
        ...
        <div className="w-full flex flex-col items-center">
          ...
          <div className="w-full max-w-[1080px] grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 pb-12">
            <div className="bg-zinc-900/50 border border-white/5 rounded-2xl p-4 flex flex-col items-center gap-3">
              ...
              <div className="flex gap-2 w-full">
                <Button
                  onClick={() => downloadPng("desktop")}
                  variant="outline"
                  className="flex-1 bg-zinc-950 border-white/10 hover:bg-zinc-800 text-zinc-300"
                >
                  <Download className="w-4 h-4 mr-2" /> Save
                </Button>
                <Button
                  onClick={() => copyPng("desktop")}
                  variant="outline"
                  className="flex-1 bg-zinc-950 border-white/10 hover:bg-zinc-800 text-zinc-300"
                >
                  <Copy className="w-4 h-4 mr-2" /> Copy
